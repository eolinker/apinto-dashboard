...
type IWarnHistoryIStore interface {
	store.IBaseStore[monitor_entry.WarnHistory]
...
type WarnHistoryStore struct {
	*store.BaseStore[monitor_entry.WarnHistory]
}

func newWarnHistoryIStore(db store.IDB) IWarnHistoryIStore {
	return &WarnHistoryStore{BaseStore: store.CreateStore[monitor_entry.WarnHistory](db)}
}
