...
type IWarnStrategyIStore interface {
	store.IBaseStore[monitor_entry.WarnStrategy]
...
type warnStrategyStore struct {
	*store.BaseStore[monitor_entry.WarnStrategy]
}

func newWarnStrategyIStore(db store.IDB) IWarnStrategyIStore {
	return &warnStrategyStore{BaseStore: store.CreateStore[monitor_entry.WarnStrategy](db)}
}
