...
type IUserRoleStore interface {
	store.IBaseStore[user_entry.UserRole]
...
type userRoleStore struct {
	*store.BaseStore[user_entry.UserRole]
}

func newUserRoleStore(db store.IDB) IUserRoleStore {
	return &userRoleStore{BaseStore: store.CreateStore[user_entry.UserRole](db)}
}
