...
type IAuditLogStore interface {
	IBaseStore[audit_entry.AuditLog]
...
type auditLogStore struct {
	*BaseStore[audit_entry.AuditLog]
}

func newAuditLogStore(db IDB) IAuditLogStore {
	return &auditLogStore{BaseStore: CreateStore[audit_entry.AuditLog](db)}
}
...
func (c *auditLogStore) GetLogsByCondition(ctx context.Context, namespaceID, operateType int, kind, keyword string, start, end int64, pageNum, pageSize int) ([]*audit_entry.AuditLog, int, error) {
	...
	err := db.Model(logs).Count(&count).Order("start_time DESC").Limit(pageSize).Offset(page_entry.PageIndex(pageNum, pageSize)).Find(&logs).Error
	if err != nil {
		return nil, 0, err
	}
