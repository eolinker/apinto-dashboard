...
describe('DeployClusterCreateComponent test', () => {
    ...
    const spyService = jest
      ...
      .mockReturnValue(
        of({
          ...
          data: {
            nodes: [
              { name: 'name1', adminAddr: 'addr1', serviceAddr: 'addr1', status: 'NOTRUNNING' },
              { name: 'name2', adminAddr: 'addr2', serviceAddr: 'addr2', status: 'NOTRUNNING' },
              { name: 'name3', adminAddr: 'addr3', serviceAddr: 'addr3', status: 'RUNNING' },
              { name: 'name4', adminAddr: 'addr4', serviceAddr: 'addr4', status: 'RUNNING' }
            ],
            source: 'tttttttttttt',
            isUpdate: true
          }
  ...
  it('##saveCluster successfully', () => {
    ...
    expect(spyRouterNavigate).toHaveBeenCalledWith(['/', 'deploy', 'cluster', 'content'], { queryParams: { clusterName: 'test123_' } })
  })
