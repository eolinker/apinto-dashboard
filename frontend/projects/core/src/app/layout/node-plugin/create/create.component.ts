 ...
 * @LastEditTime: 2023-12-15 17:47:19
 * @FilePath: \apinto\projects\core\src\app\layout\node-plugin\create\create.component.ts
 */
...
export class DeployPluginCreateComponent implements OnInit {
        ...
        (resp: {
          ...
          if (resp.code === 0) {
            ...
            setFormValue(this.validateForm, {
              ...
              extended: resp.data.plugin!.title,
  ...
  getExtendsList () {
    this.api.get('plugin/extendeds').subscribe((resp:{code:number, data:{extendeds:{name:string, title:string}[]}, msg:string}) => {
      if (resp.code === 0) {
