...
import { MonitorProxyData } from '../../area/total/total.component'
import { TableQueryData } from '../api-table/api-table.component'

@Component({
  selector: 'eo-ng-monitor-alarm-proxy-path-table',
  template: `
  <div *ngIf="showSearch"
...
export class ProxyPathTableComponent implements OnInit {
  @Input() nzScrollY:number = 0
  @Input() type:string = 'UpstreamProxyDetail'
  ...
  changeDisplayData (keyword:string) {
    ...
    if (this.proxyTableList?.length > 0) {
      this.proxyTableDisplayList = this.proxyTableList.filter((item:MonitorProxyData) => (
        item.proxy_path.includes(keyword)
      ))
      this.queryData.total = this.proxyTableDisplayList.length
    }
  }
