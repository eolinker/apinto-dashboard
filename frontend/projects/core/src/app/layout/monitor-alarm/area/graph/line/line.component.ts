...
import { InvokeData, MessageData } from '../../../types/types'
...
export class MonitorLineGraphComponent implements OnInit {
  ...
  @Input() type:'invoke'|'invoke-service'|'traffic'|'' = 'invoke'
  ...
  changeInterval (data:any, yAxis:any, legendSeleted?:any) {
    ...
    for (const key of Object.keys(data)) {
      if (key !== 'date' && key !== 'requestRate' && key !== 'proxyRate') {
      ...
      case 'invoke': {
        if (!this.compare) {
          this.lineChartOption = {
            ...
            series: [
              { type: 'line', symbol: 'none', name: '请求总数', data: (this.lineData as InvokeData).requestTotal, yAxisIndex: 0 },
              { type: 'line', symbol: 'none', name: '请求成功率', data: (this.lineData as InvokeData).requestRate?.map((x) => Number((Number(x) * 100).toFixed(2))) || [], yAxisIndex: 1 },
              { type: 'line', symbol: 'none', name: '转发总数', data: (this.lineData as InvokeData).proxyTotal, yAxisIndex: 0 },
              { type: 'line', symbol: 'none', name: '转发成功率', data: (this.lineData as InvokeData).proxyRate?.map((x) => Number((Number(x) * 100).toFixed(2))) || [], yAxisIndex: 1 },
              { type: 'line', symbol: 'none', lineStyle: { type: 'dashed' }, name: '状态码4xx数', data: (this.lineData as InvokeData).status4xx, yAxisIndex: 0 },
              { type: 'line', symbol: 'none', lineStyle: { type: 'dashed' }, name: '状态码5xx数', data: (this.lineData as InvokeData).status5xx, yAxisIndex: 0 }
            ]
          }
        } else {
          this.lineChartOption = {
            ...
            series: [
              { type: 'line', symbol: 'none', name: '请求总数', data: (this.lineData as InvokeData).requestTotal, xAxisIndex: 0, yAxisIndex: 0 },
              { type: 'line', symbol: 'none', name: '请求成功率', data: (this.lineData as InvokeData).requestRate?.map((x) => Number((Number(x) * 100).toFixed(2))) || [], xAxisIndex: 0, yAxisIndex: 1 },
              { type: 'line', symbol: 'none', name: '转发总数', data: (this.lineData as InvokeData).proxyTotal, xAxisIndex: 0, yAxisIndex: 0 },
              { type: 'line', symbol: 'none', name: '转发成功率', data: (this.lineData as InvokeData).proxyRate?.map((x) => Number((Number(x) * 100).toFixed(2))) || [], xAxisIndex: 0, yAxisIndex: 1 },
              { type: 'line', lineStyle: { type: 'dashed' }, symbol: 'none', name: '状态码4xx数', data: (this.lineData as InvokeData).status4xx, xAxisIndex: 0, yAxisIndex: 0 },
              { type: 'line', lineStyle: { type: 'dashed' }, symbol: 'none', name: '状态码5xx数', data: (this.lineData as InvokeData).status5xx, xAxisIndex: 0, yAxisIndex: 0 },
              { type: 'line', symbol: 'none', name: '请求总数', data: (this.compareData as InvokeData).requestTotal, xAxisIndex: 1, yAxisIndex: 2 },
              { type: 'line', symbol: 'none', name: '请求成功率', data: (this.compareData as InvokeData).requestRate?.map((x) => Number((Number(x) * 100).toFixed(2))) || [], xAxisIndex: 1, yAxisIndex: 3 },
              { type: 'line', symbol: 'none', name: '转发总数', data: (this.compareData as InvokeData).proxyTotal, xAxisIndex: 1, yAxisIndex: 2 },
              { type: 'line', symbol: 'none', name: '转发成功率', data: (this.compareData as InvokeData).proxyRate?.map((x) => Number((Number(x) * 100).toFixed(2))) || [], xAxisIndex: 1, yAxisIndex: 3 },
              { type: 'line', lineStyle: { type: 'dashed' }, symbol: 'none', name: '状态码4xx数', data: (this.compareData as InvokeData).status4xx, xAxisIndex: 1, yAxisIndex: 2 },
              { type: 'line', lineStyle: { type: 'dashed' }, symbol: 'none', name: '状态码5xx数', data: (this.compareData as InvokeData).status5xx, xAxisIndex: 1, yAxisIndex: 2 }
            ]
          }
        }
        break
        ...
        if (!this.compare) {
          this.lineChartOption = {
            ...
            series: [
              { type: 'line', symbol: 'none', name: '转发总数', data: (this.lineData as InvokeData).proxyTotal, yAxisIndex: 0 },
              { type: 'line', symbol: 'none', name: '转发成功率', data: (this.lineData as InvokeData).proxyRate?.map((x) => Number((Number(x) * 100).toFixed(2))) || [], yAxisIndex: 1 },
              { type: 'line', lineStyle: { type: 'dashed' }, symbol: 'none', name: '状态码4xx数', data: (this.lineData as InvokeData).status4xx, yAxisIndex: 0 },
              { type: 'line', lineStyle: { type: 'dashed' }, symbol: 'none', name: '状态码5xx数', data: (this.lineData as InvokeData).status5xx, yAxisIndex: 0 }
            ]
          }
        } else {
          this.lineChartOption = {
            ...
            series: [
              { type: 'line', symbol: 'none', name: '转发总数', data: (this.lineData as InvokeData).proxyTotal, xAxisIndex: 0, yAxisIndex: 0 },
              { type: 'line', symbol: 'none', name: '转发成功率', data: (this.lineData as InvokeData).proxyRate, xAxisIndex: 0, yAxisIndex: 1 },
              { type: 'line', symbol: 'none', lineStyle: { type: 'dashed' }, name: '状态码4xx数', data: (this.lineData as InvokeData).status4xx, xAxisIndex: 0, yAxisIndex: 0 },
              { type: 'line', symbol: 'none', lineStyle: { type: 'dashed' }, name: '状态码5xx数', data: (this.lineData as InvokeData).status5xx, xAxisIndex: 0, yAxisIndex: 0 },
              { type: 'line', symbol: 'none', name: '转发总数', data: (this.compareData as InvokeData).proxyTotal, xAxisIndex: 1, yAxisIndex: 2 },
              { type: 'line', symbol: 'none', name: '转发成功率', data: (this.compareData as InvokeData).proxyRate, xAxisIndex: 1, yAxisIndex: 3 },
              { type: 'line', symbol: 'none', lineStyle: { type: 'dashed' }, name: '状态码4xx数', data: (this.compareData as InvokeData).status4xx, xAxisIndex: 1, yAxisIndex: 2 },
              { type: 'line', symbol: 'none', lineStyle: { type: 'dashed' }, name: '状态码5xx数', data: (this.compareData as InvokeData).status5xx, xAxisIndex: 1, yAxisIndex: 2 }
            ]
          }
        }
        break
