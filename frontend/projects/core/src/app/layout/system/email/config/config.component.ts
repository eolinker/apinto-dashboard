...
export class SystemEmailConfigComponent implements OnInit {
  ...
  getEmailMessage () {
    this.api.get('warn/email').subscribe((resp:{code:number, data:{emailInfo?:EmailData}, msg:string}) => {
  ...
  save () {
    if (this.validateForm.valid) {
      ...
      if (this.editPage) {
        this.api.put('warn/email', { ...data, uuid: this.emailId }).subscribe((resp:{code:number, data:{}, msg:string}) => {
          if (resp.code === 0) {
            this.message.success(resp.msg || '修改通知邮箱成功！')
          }
        })
      } else {
        this.api.post('warn/email', { ...data }).subscribe((resp:{code:number, data:{}, msg:string}) => {
          if (resp.code === 0) {
