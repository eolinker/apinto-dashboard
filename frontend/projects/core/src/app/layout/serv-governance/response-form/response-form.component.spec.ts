...
describe('ResponseFormComponent test as editPage is false', () => {
  ...
  beforeEach(async () => {
    ...
    component._responseForm = new UntypedFormGroup({
      status_code: new UntypedFormControl([200, [Validators.required, Validators.pattern(/^[1-9]{1}\d{2}$/)]]),
      content_type: new UntypedFormControl(['application/json', [Validators.required]]),
  ...
  it('##getContentTypeList with success return', () => {
    ...
    const spyService = jest.spyOn(httpCommonService, 'get').mockReturnValue(
      of({
        code: 0,
        data: { items: [{ content_type: 'test1', body: 'test1' }, { content_type: 'test2', body: 'test2' }, { content_type: 'test3', body: 'test3' }] }
      })
    )
    // @ts-ignore
    ...
    component._responseForm.controls['content_type'].setValue('test1')
