...
describe('VisitCreateComponent test as editPage is false', () => {
  ...
  it('##getStrategyMessage with success return (with optional attr)', () => {
    ...
    const mockValue2: any = {
      ...
      data: {
        strategy: {
          ...
          config: {
            visit_rule: 'allow',
            influence_sphere: [{ name: 'testG', uuid: '12321' }],
            continue: false
          }
        }
      }
    ...
    expect(component.filterNamesSet).toStrictEqual(new Set(['testF', 'testG']))
    expect(component.filterShowList).toStrictEqual([
      { name: 'testF', uuid: '123' }
    ])
    expect(component.influenceShowList).toStrictEqual([
      { name: 'testG', uuid: '12321' }
    ])
    expect(spyService).toHaveBeenCalled()
  })
  ...
  it('##saveStrategy with success return', () => {
    ...
    component.validateForm.controls['continue'].setValue(false)
    ...
    component.validateForm.controls['name'].setValue('test')
    component.validateForm.controls['priority'].setValue(222)
    component.validateForm.controls['continue'].setValue(null)
    component.editPage = false
    component.saveStrategy()
    fixture.detectChanges()
    ...
    component.createStrategyForm.config.visit_rule = ''
    component.saveStrategy()
    fixture.detectChanges()
