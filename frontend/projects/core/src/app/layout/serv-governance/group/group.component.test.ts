...
describe('flow-control-group e2e test', () => {
  ...
  it('点击分组中的二级节点(集群), 被选中的节点背景色变化为--item-hover-background-color', async () => {
    await page.locator('nz-tree-indent + nz-tree-node-title').nth(2).click()
    await expect(page.locator('nz-tree-indent + nz-tree-node-title').nth(1).getAttribute('styles')).not.toContain('{background-color:var(--primary-color)}')
    await expect(page.locator('nz-tree-indent + nz-tree-node-title').nth(2).getAttribute('styles')).toContain('{background-color:var(--primary-color)}')
  })
  it('鼠标悬浮的节点背景色变化为--item-hover-background-color', async () => {
