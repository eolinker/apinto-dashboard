...
describe('服务治理-缓存策略 e2e test', () => {
  ...
  it('填写所有必填项，点击保存，页面出现消息提示, 当消息提示为成功时, 页面返回列表页, 否则停留本页', async () => {
    ...
    await page.locator('nz-form-control').filter({ hasText: '必填项单位：s，最小值：1' }).getByRole('alert').click()

    await page.getByPlaceholder('请输入首字母为英文，英文数字下划线任意一种组合').fill('testForE2e')
    await page.locator('.fix-buttom-group').getByRole('button', { name: '保存' }).click()

    await page.getByPlaceholder('请输入有效时间').click()
    await page.getByPlaceholder('请输入有效时间').fill('1')
    await page.locator('.fix-buttom-group').getByRole('button', { name: '保存' }).click()
  ...
  it('点击列表中的查看按钮, 策略名称&筛选条件列表&限流规则不为空, 保存按钮为primary, 点击保存出现全局消息提示, 并返回列表页', async () => {
    ...
    await page.getByText('缓存策略 / testForE2e /').click()
  ...
  it('输入框大小, 按钮的大小和颜色, 表格的大小, 限流规则的背景色', async () => {
    ...
    const visitRuleInput = await page.getByPlaceholder('请输入有效时间')
    ...
    const filterBtn1 = await page.getByRole('button', { name: '添加条件' }).first()
