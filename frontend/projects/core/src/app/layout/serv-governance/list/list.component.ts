...
export class ListComponent implements OnInit {
  ...
  @Output() changeToContent:EventEmitter<any> = new EventEmitter()
  clustersOptions : Array<{label:string, value:string}> = []
  cluster:string = ''
  nzDisabled:boolean = false
  drawerPublishRef:NzModalRef | undefined
  priorityMap : Map<number|string, any> = new Map() // 优先级与对应策略的map，随着修改而改变
  priorityOldMap: Map<number|string, any> = new Map() // 策略uuid与优先级的map，接口赋值后不变，用于提交优先级修改时做对比
  prioritySaveMap:any = {} // 用于提交的优先级，键为uuid，值为优先级
  ...
  strategiesTableHeadName:Array<any> = [
    {
      title: '策略名称'
    },
    {
  ...
  ngOnInit (): void {
    ...
    // 当左侧集群被选中时,cluster_name参数会发生变化,随之获取新的策略列表
  ...
  changeEditingPriority (e:Event, value:any) {
    e?.stopPropagation()
    if (!value) {
  ...
  changePriorityMap (editingPriority:number|string, priority:string|number, uuid:string) {
    let tempStrategy:any = null
    const tempMapArray:Array<any> = this.priorityMap.get(editingPriority)?.filter((item:any) => {
      if (item?.uuid === uuid) {
        tempStrategy = item
      }
  ...
  changePriority () {
    ...
    this.api.post('strategy/' + this.strategyType + '/priority', this.prioritySaveMap, { cluster_name: (this.clusterName || '') }).subscribe((resp:any) => {
      if (resp.code === 0) {
  ...
  getStrategiesList () {
    if (this.clusterName) {
      this.api.get('strategies/' + this.strategyType, { cluster_name: this.clusterName || '' }).subscribe((resp:any) => {
        if (resp.code === 0) {
  ...
  // 编辑策略
  editStrategy (uuid:any) {
    this.router.navigate(['/', 'serv-governance', this.strategyType, 'message', this.clusterName, uuid])
  }

  // 当参数为true时,停用策略,false为启用
  stopStrategy (e:Event, item:any, isStop:boolean) {
    e.stopPropagation()
    this.api.patch('strategy/' + this.strategyType + '/stop', { is_stop: isStop }, { uuid: (item.uuid || ''), cluster_name: (this.clusterName || '') }).subscribe((resp:any) => {
      if (resp.code === 0) {
        item.is_stop = isStop
  ...
  deleteStrategy (item:any) {
    this.api.delete('strategy/' + this.strategyType, { uuid: (item.uuid || ''), cluster_name: (this.clusterName || '') }).subscribe((resp:any) => {
      if (resp.code === 0) {
  ...
  // 恢复策略
  recoverStrategy (item:any, e?:Event) {
    e?.stopPropagation()
    this.api.patch('strategy/' + this.strategyType + '/restore', {}, { uuid: (item.uuid || ''), cluster_name: (this.clusterName || '') }).subscribe((resp:any) => {
      if (resp.code === 0) {
        this.getStrategiesList()
