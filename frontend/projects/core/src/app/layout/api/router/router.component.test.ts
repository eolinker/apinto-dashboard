describe('API管理 e2e test', () => {
  ...
  it('从分组内部的添加api进入新建api页面，其中分组栏不为空，检查表单页所有输入框样式，点击取消，返回列表页', async () => {
    ...
    // 请求路径
    const requestPathInput = await page.locator('#requestPath')
    ...
    // 转发上游路径
    const upstreamPathInput = await page.locator('#proxyPath')
  ...
  it('点击新建API，清空必输项，表单无法保存；逐个填写必填项，直至表单可以提交，将返回列表页', async () => {
    ...
    await page.getByRole('button', { name: '保存' }).click()
    await page.locator('#requestPath').click()
    await page.locator('#requestPath').fill('test')
    ...
    await page.getByRole('button', { name: '保存' }).click()
    await page.locator('#proxyPath').click()
    await page.locator('#proxyPath').click()
    await page.locator('#proxyPath').fill('/test')
    await page.locator('#timeout').click()
    await page.getByRole('button', { name: '保存' }).click()
    await page.locator('#timeout').click()
