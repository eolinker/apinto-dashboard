...
type IGlobalVariableStore interface {
	store.IBaseStore[variable_entry.Variables]
...
type globalVariableStore struct {
	*store.BaseStore[variable_entry.Variables]
}

func newGlobalVariableStore(db store.IDB) IGlobalVariableStore {
	return &globalVariableStore{BaseStore: store.CreateStore[variable_entry.Variables](db)}
}
...
func (g *globalVariableStore) GetList(ctx context.Context, pageNum, pageSize, namespaceID int, searchName string) ([]*variable_entry.Variables, int, error) {
	...
	if pageNum > 0 && pageSize > 0 {
		err := db.Model(variables).Count(&count).Order("create_time asc").Limit(pageSize).Offset(store.PageIndex(pageNum, pageSize)).Find(&variables).Error
		if err != nil {
			return nil, 0, err
		}
	} else {
