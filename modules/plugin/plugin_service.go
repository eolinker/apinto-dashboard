...
type IPluginService interface {
	...
	GetEnumList(ctx context.Context, namespaceId int) ([]*plugin_model.PluginEnum, error)
	...
	InsertBuiltin(ctx context.Context, namespaceId int, plugins []*plugin_model.PluginBuilt) error //新增内置插件
	Sort(ctx context.Context, namespaceId, operator int, names []string) error
	GetExtendersCache(ctx context.Context, namespaceId int) ([]*plugin_model.ExtenderInfo, error)
}
