...
func newMonitorStatisticsCache() monitor.IMonitorStatisticsCache {
	val := new(monitorStatisticsCache)
	bean.Autowired(&val.commonCache)
	return val
}

func (m *monitorStatisticsCache) GetStatisticsCache(ctx context.Context, partitionId string, start, end time.Time, groupBy string, wheres []model.MonWhereItem, limit int) (map[string]model.MonCommonData, error) {
	...
	valMap := make(map[string]model.MonCommonData)
	for k, v := range maps {
		commonData := &model.MonCommonData{}
...
func (m *monitorStatisticsCache) SetStatisticsCache(ctx context.Context, partitionId string, start, end time.Time, groupBy string, wheres []model.MonWhereItem, limit int, values map[string]model.MonCommonData) error {
	key := fmt.Sprintf("monitor:statistics:%s:%d_%d:%s:%s:%d", partitionId, start.Unix(), end.Unix(), groupBy, m.formatWhereKey(wheres), limit)
...
func (m *monitorStatisticsCache) GetTrendCache(ctx context.Context, partitionId string, start, end time.Time, wheres []model.MonWhereItem) (*model.MonCallCountInfo, error) {
	...
	val := new(model.MonCallCountInfo)
...
func (m *monitorStatisticsCache) SetTrendCache(ctx context.Context, partitionId string, start, end time.Time, wheres []model.MonWhereItem, value *model.MonCallCountInfo) error {
	key := fmt.Sprintf("monitor:trend:%s:%d_%d:%s", partitionId, start.Unix(), end.Unix(), m.formatWhereKey(wheres))
...
func (m *monitorStatisticsCache) GetCircularMap(ctx context.Context, partitionId string, start, end time.Time, wheres []model.MonWhereItem) (*model.CircularDate, *model.CircularDate, error) {
	...
	requestDate := new(model.CircularDate)
	...
	proxyDate := new(model.CircularDate)
...
func (m *monitorStatisticsCache) SetCircularMap(ctx context.Context, partitionId string, start, end time.Time, wheres []model.MonWhereItem, request, proxy *model.CircularDate) error {
	key := fmt.Sprintf("monitor:circular_map:%s:%d_%d:%s", partitionId, start.Unix(), end.Unix(), m.formatWhereKey(wheres))
...
func (m *monitorStatisticsCache) GetMessageTrend(ctx context.Context, partitionId string, start, end time.Time, wheres []model.MonWhereItem) (*model.MessageTrend, error) {
...
func (m *monitorStatisticsCache) SetMessageTrend(ctx context.Context, partitionId string, start, end time.Time, wheres []model.MonWhereItem, value *model.MessageTrend) error {
	key := fmt.Sprintf("monitor:message_trend:%s:%d_%d:%s", partitionId, start.Unix(), end.Unix(), m.formatWhereKey(wheres))
...
func (m *monitorStatisticsCache) formatWhereKey(wheres []model.MonWhereItem) string {

	whereMap := make(map[string]model.MonWhereItem)
