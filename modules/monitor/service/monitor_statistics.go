...
type monitorStatistics struct {
	...
	monitorStatisticsCache monitor.IMonitorStatisticsCache
}

func newMonitorStatistics() monitor.IMonitorStatistics {
...
func (m *monitorStatistics) WarnStatistics(ctx context.Context, namespaceId int, partitionId string, start, end time.Time, group string, quotaType warn_model.QuotaType, wheres []model.MonWhereItem) (map[string]float64, error) {
	/*
...
func (m *monitorStatistics) calculateWarnStatistics(quotaType warn_model.QuotaType, data map[string]*monitor_entry.FluxWarnStatistics) map[string]float64 {
	results := make(map[string]float64)

	switch quotaType {
	case warn_model.QuotaTypeReqFailCount:
		for key, statistic := range data {
			results[key] = float64(statistic.Total - statistic.Success)
		}
	case warn_model.QuotaTypeReqFailRate:
		for key, statistic := range data {
	...
	case warn_model.QuotaTypeReqStatus4xx:
		for key, statistic := range data {
			results[key] = float64(statistic.S4xx)
		}
	case warn_model.QuotaTypeReqStatus5xx:
		for key, statistic := range data {
			results[key] = float64(statistic.S5xx)
		}
	case warn_model.QuotaTypeProxyFailCount:
		for key, statistic := range data {
			results[key] = float64(statistic.ProxyTotal - statistic.ProxySuccess)
		}
	case warn_model.QuotaTypeProxyFailRate:
		for key, statistic := range data {
	...
	case warn_model.QuotaTypeProxyStatus4xx:
		for key, statistic := range data {
			results[key] = float64(statistic.ProxyS4xx)
		}
	case warn_model.QuotaTypeProxyStatus5xx:
		for key, statistic := range data {
			results[key] = float64(statistic.ProxyS5xx)
		}
	case warn_model.QuotaTypeReqMessage:
		for key, statistic := range data {
			results[key] = float64(statistic.TotalRequest) / 1024
		}
	case warn_model.QuotaTypeRespMessage:
		for key, statistic := range data {
...
func (m *monitorStatistics) getStatisticsFilterByQuota(quotaType warn_model.QuotaType) (*monitor_entry.StatisticsFilterConf, error) {
	...
	switch quotaType {
	case warn_model.QuotaTypeReqFailCount, warn_model.QuotaTypeReqFailRate:
	...
	case warn_model.QuotaTypeReqStatus5xx:
		...
		return filterConf, nil
	case warn_model.QuotaTypeProxyFailCount, warn_model.QuotaTypeProxyFailRate:
