...
import (
	...
	warn_entry "github.com/eolinker/apinto-dashboard/modules/warn/warn-entry"
...
func newWarnStrategyService() warn.IWarnStrategyService {
...
func (w *warnStrategyService) WarnStrategyAll(ctx context.Context, namespaceId, status int) ([]*warn_model.WarnStrategy, error) {
	strategies, err := w.warnStrategyStore.GetAll(ctx, namespaceId, status)
	if err != nil {
		return nil, err
	}

	result := make([]*warn_model.WarnStrategy, 0, len(strategies))
	for _, warnStrategy := range strategies {

		warnStrategyConfig := new(warn_model.WarnStrategyConfig)
		...
		result = append(result, &warn_model.WarnStrategy{
			...
			Quota:              warn_model.QuotaType(warnStrategy.Quota),
...
func (w *warnStrategyService) WarnStrategyListPage(ctx context.Context, namespaceId int, query *warn_model.QueryWarnStrategyParam) ([]*warn_model.WarnStrategy, int64, error) {
	...
	userIds := common.SliceToSliceIds(list, func(t *warn_entry.WarnStrategy) int {
		return t.Operator
	})
	...
	result := make([]*warn_model.WarnStrategy, 0, len(list))
...
func (w *warnStrategyService) WarnStrategyByUuid(ctx context.Context, _ int, uuid string) (*warn_model.WarnStrategy, error) {
	warnStrategy, err := w.warnStrategyStore.GetByUuid(ctx, uuid)
	if err != nil {
		return nil, err
	}
	...
	result := &warn_model.WarnStrategy{
		Uuid:               warnStrategy.UUID,
		Title:              warnStrategy.Title,
		Desc:               warnStrategy.Desc,
		IsEnable:           warnStrategy.IsEnable,
...
func (w *warnStrategyService) CreateWarnStrategy(ctx context.Context, namespaceId, userId int, input *warn_model.WarnStrategy) error {
	...
	strategy := &warn_entry.WarnStrategy{
	...
	return w.warnStrategyStore.Transaction(ctx, func(txCtx context.Context) error {
		...
		if len(channels) > 0 {
			//往引用表插入所引用的通知渠道
			channelIds := common.SliceToSliceIds(channels, func(t *notice_model.NoticeChannel) int {
				return t.Id
			})

			if err := w.quoteStore.Set(txCtx, strategy.Id, quote_entry.QuoteKindTypeWarnStrategy, quote_entry.QuoteTargetKindTypeNoticeChannel, channelIds...); err != nil {
				return err
			}
		}
...
func (w *warnStrategyService) UpdateWarnStrategy(ctx context.Context, namespaceId, userId int, input *warn_model.WarnStrategy) error {
