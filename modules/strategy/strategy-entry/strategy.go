...
type Strategy struct {
	...
	ClusterId   int       `gorm:"type:int(11);size:11;not null;column:cluster;index:priority_type;comment:集群ID" json:"cluster_id,omitempty"`
	Type        string    `gorm:"size:50;default null;column:type;index:priority_type;comment:策略类型" json:"type,omitempty"`
	...
	Priority    int       `gorm:"type:int(11);size:11;not null;column:priority;index:priority_type;comment:优先级" json:"priority,omitempty"`
	IsStop      bool      `gorm:"type:tinyint(1);not null;size:1;column:is_stop;comment:是否停用" json:"is_stop,omitempty"` //true 停用  false启用
	IsDelete    bool      `gorm:"type:tinyint(1);not null;size:1;column:is_delete;comment:是否已经删除(软删除)" json:"is_delete,omitempty"`
