...
type Schema struct {
	...
	XReactions           interface{}         `json:"x-reactions,omitempty"`
	XComponentProps      interface{}         `json:"x-component-props,omitempty"`
...
func generateFromField(f reflect.StructField, mode Mode) (name string, required bool, s *Schema, err error) {
	...
	if tag, ok := f.Tag.Lookup("x-decorator"); ok {
		s.XDecorator = tag
	}

	if tag, ok := f.Tag.Lookup("x-component"); ok {
		s.XDecorator = tag
	}

	if tag, ok := f.Tag.Lookup("x-reactions"); ok {
		conf := make(map[string]interface{})
		err = json.Unmarshal([]byte(tag), &conf)
		if err != nil {
			return name, required, nil, err
		}
		s.XReactions = conf
	...
	if tag, ok := f.Tag.Lookup("x-component-props"); ok {
		conf := make(map[string]interface{})
		err = json.Unmarshal([]byte(tag), &conf)
		if err != nil {
			return name, required, nil, err
