...
type monitorController struct {
	...
	monStatistics  monitor_service.IMonitorStatistics
	apiService     service2.IAPIService
	appService     application_service.IApplicationService
...
func (m *monitorController) getPartitionInfo(ginCtx *gin.Context) {
	...
	if err != nil {
		ginCtx.JSON(http.StatusOK, dto.NewErrorResult(fmt.Sprintf("Get monitor partition info fail. err: %s", err)))
		return
	}
...
func (m *monitorController) createPartition(ginCtx *gin.Context) {
	...
	if err != nil {
		ginCtx.JSON(http.StatusOK, dto.NewErrorResult(fmt.Sprintf("Create monitor partition fail. err: %s", err)))
		return
	}
...
func (m *monitorController) editPartition(ginCtx *gin.Context) {
	...
	if err != nil {
		ginCtx.JSON(http.StatusOK, dto.NewErrorResult(fmt.Sprintf("Update monitor partition fail. err: %s", err)))
		return
	}
...
func (m *monitorController) overviewSummary(ginCtx *gin.Context) {
	...
	if input.Start <= 0 || input.End <= 0 || input.Start >= input.End {
		ginCtx.JSON(http.StatusOK, dto.NewErrorResult(fmt.Sprintf("timestamp is illegal.")))
		return
	}
	...
	wheres := make([]monitor_model.MonWhereItem, 0, len(input.Clusters))
	if len(clusterIds) > 0 {
...
func (m *monitorController) overviewTop(ginCtx *gin.Context) {
	...
	default:
		ginCtx.JSON(http.StatusOK, dto.NewErrorResult(fmt.Sprintf("data_type is illegal.")))
		return
	}
...
func (m *monitorController) getStatistics(ginCtx *gin.Context) {
	...
	case "app":
		...
		if len(input.AppIds) > 0 {
			...
			if err != nil {
				ginCtx.JSON(http.StatusOK, dto.NewErrorResult("query apps error = "+err.Error()))
				return
			}
		} else if detailsType == "" {
	...
	case "service":
		...
		if len(input.Services) > 0 {
			...
			if err != nil {
				ginCtx.JSON(http.StatusOK, dto.NewErrorResult("query services error = "+err.Error()))
				return
			}
		} else {
	...
	default:
		ginCtx.JSON(http.StatusOK, dto.NewErrorResult("参数错误"))
		return
	}
	...
	maps := make(map[string]monitor_model.MonCommonData)
	...
	case "app":
		if len(maps) > 0 {
			...
			if err != nil {
				ginCtx.JSON(http.StatusOK, dto.NewErrorResult("query app error = "+err.Error()))
				return
			}
		}
...
func (m *monitorController) getStatisticsDetails(ginCtx *gin.Context) {
	...
	case "api":
		if input.ApiId == "" {
			ginCtx.JSON(http.StatusOK, dto.NewErrorResult("api_id can't be null. "))
			return
		}
	case "app":
		if input.AppId == "" {
			ginCtx.JSON(http.StatusOK, dto.NewErrorResult("app_id can't be null. "))
			return
		}
	case "addr":
		if input.Addr == "" {
			ginCtx.JSON(http.StatusOK, dto.NewErrorResult("addr can't be null. "))
			return
		}
	}
	...
	values := &monitor_model.MonCallCountInfo{}
...
func (m *monitorController) getWheres(ginCtx *gin.Context, namespaceId int, input *monitor_dto.MonCommonInput, clusterIds []string) ([]monitor_model.MonWhereItem, error) {
...
func newMonCommonData(input monitor_model.MonCommonData) monitor_dto.MonCommonData {
