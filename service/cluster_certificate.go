...
type IClusterCertificateService interface {
	...
	QueryList(ctx context.Context, namespaceId int, clusterName string) ([]*model.ClusterCertificate, error)
	DeleteById(ctx context.Context, namespaceId int, clusterName string, id int) error
}
type clusterCertificateService struct {
	clusterCertificateStore store.IClusterCertificateStore
	clusterService          IClusterService
	userInfoService         IUserInfoService
	apintoClient            IApintoClient
}

func newClusterCertificateService() IClusterCertificateService {
...
// QueryList 查询证书列表
func (c *clusterCertificateService) QueryList(ctx context.Context, namespaceId int, clusterName string) ([]*model.ClusterCertificate, error) {
	...
	modelList := make([]*model.ClusterCertificate, 0, len(list))
	for _, certificate := range list {
		...
		modelList = append(modelList, &model.ClusterCertificate{ClusterCertificate: certificate, OperatorName: operatorName})
	}
