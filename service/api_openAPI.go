...
func newAPIOpenAPIService() IAPIOpenAPIService {
...
func (a *apiOpenAPIService) SyncImport(ctx context.Context, namespaceID, appID int, data *dto.SyncImportData) ([]*apimodel.ImportAPIListItem, error) {
	...
	return items, a.apiStore.Transaction(ctx, func(txCtx context.Context) error {
		...
		if serviceNotExit {
			...
			staticConf := make([]*model.ServiceStaticConf, 0, len(data.Server.Nodes))
			for _, node := range data.Server.Nodes {
				...
				staticConf = append(staticConf, &model.ServiceStaticConf{
					Addr:   strings.TrimSpace(node.Url),
					Weight: node.Weight,
				})
			}
			config := &model.ServiceStaticDriverConf{
				UseVariable: false,
...
func (a *apiOpenAPIService) GetSyncImportInfo(ctx context.Context, namespaceID int) ([]*model.ApiOpenAPIGroups, []*model.ApiOpenAPIService, []string, error) {
	//组装api分组
	...
	apiGroups := make([]*model.ApiOpenAPIGroups, 0, len(groups.CommonGroup))
	for _, commonGroup := range groups.CommonGroup {
		group := &model.ApiOpenAPIGroups{
	...
	serviceList := make([]*model.ApiOpenAPIService, 0, st)
	for _, s := range services {
		item := &model.ApiOpenAPIService{
			Name: s.Name,
			Desc: s.Desc,
		}
...
func (a *apiOpenAPIService) subGroup(val *model.ApiOpenAPIGroups, list []*model.CommonGroup) {
	if len(list) == 0 {
		return
	}
	for _, group := range list {
		commonGroup := &model.ApiOpenAPIGroups{
