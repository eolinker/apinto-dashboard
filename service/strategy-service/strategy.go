...
import (
	...
	"github.com/eolinker/apinto-dashboard/store/strategy-store"
...
type IStrategyService[T any, K any] interface {
	GetList(ctx context.Context, namespaceId int, clusterName string) ([]*openapi_model.Strategy, error)
	GetInfo(ctx context.Context, namespaceId int, uuid string) (*openapi_model.StrategyInfoOutput[K], *openapi_model.ExtenderData, error)
	...
	ToPublish(ctx context.Context, namespaceId int, clusterName string) ([]*openapi_model.StrategyToPublish[T], error)
	PublishHistory(ctx context.Context, namespaceId, pageNum, pageSize int, clusterName string) ([]*openapi_model.StrategyPublishHistory, int, error)
...
func (s *strategyService[T, K]) GetList(ctx context.Context, namespaceId int, clusterName string) ([]*openapi_model.Strategy, error) {
	...
	resList := make([]*openapi_model.Strategy, 0, len(strategies))
	for _, strategy := range strategies {
		...
		resList = append(resList, &openapi_model.Strategy{
			Strategy:    strategy,
			Version:     version,
			Filters:     filters,
...
func (s *strategyService[T, K]) GetInfo(ctx context.Context, namespaceId int, uuid string) (*openapi_model.StrategyInfoOutput[K], *openapi_model.ExtenderData, error) {
	...
	//拼装筛选条件所需要的信息
	extenderData := &openapi_model.ExtenderData{
		Api:         make(map[string]*openapi_model.RemoteApis),
		Service:     make(map[string]*openapi_model.RemoteServices),
		Application: make(map[string]*openapi_model.RemoteApplications),
	}
	filters := make([]*openapi_model.FilterOutput, 0, len(version.Filters))
	for _, f := range version.Filters {
		filter := &openapi_model.FilterOutput{
			Name:   f.Name,
			Values: f.Values,
		}
		if len(f.Values) == 0 {
		...
		case enum.FilterApplication:
			...
			if f.Values[0] == enum.FilterValuesALL {
				...
				for i, app := range apps {
					extenderData.Application[app.IdStr] = &openapi_model.RemoteApplications{
						Name: app.Name,
						Uuid: app.IdStr,
						Desc: app.Desc,
					}
	...
	input := &openapi_model.StrategyInfoOutput[T]{
...
func (s *strategyService[T, K]) ToPublish(ctx context.Context, namespaceId int, clusterName string) ([]*openapi_model.StrategyToPublish[T], error) {
	...
	resList := make([]*openapi_model.StrategyToPublish[T], 0)
	for _, strategy := range strategies {
		//已上线的状态过滤掉
		if strategy.Status == enum.StrategyOnlineStatusGOONLINE {
			continue
		}
		resList = append(resList, &openapi_model.StrategyToPublish[T]{
...
func (s *strategyService[T, K]) PublishHistory(ctx context.Context, namespaceId, pageNum, pageSize int, clusterName string) ([]*openapi_model.StrategyPublishHistory, int, error) {
	cluster, err := s.clusterService.GetByNamespaceByName(ctx, namespaceId, clusterName)
	if err != nil {
	...
	resp := make([]*openapi_model.StrategyPublishHistory, 0, len(list))

	for _, history := range list {

		details := make([]*openapi_model.StrategyPublishHistoryDetails, 0)
		for _, publish := range history.Publish {
			details = append(details, &openapi_model.StrategyPublishHistoryDetails{
		...
		resp = append(resp, &openapi_model.StrategyPublishHistory{
			Id:         history.Id,
...
func (s *strategyService[T, K]) Publish(ctx context.Context, namespaceId, operator int, clusterName string, input *strategy_dto.StrategyPublish) error {
	...
	publishes := make([]*openapi_model.StrategyToPublish[T], 0)
	...
	//需要物理删除的策略
	deleteStrategyMaps := common.Map[int, *openapi_model.StrategyToPublish[T]]{}
