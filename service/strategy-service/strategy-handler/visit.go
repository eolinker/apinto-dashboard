...
func (t *visitHandler) FormatOut(ctx context.Context, namespaceID int, input *openapi_model.StrategyInfoOutput[strategy_entry.StrategyVisitConfig]) *openapi_model.StrategyInfoOutput[openapi_model.VisitInfoOutputConf] {
	output := new(openapi_model.StrategyInfoOutput[openapi_model.VisitInfoOutputConf])
	output.Strategy = input.Strategy
	output.Filters = input.Filters

	config := &openapi_model.VisitInfoOutputConf{
	...
	filters := make([]*openapi_model.FilterOutput, 0, len(input.Filters))
...
func NewStrategyVisitHandler(apintoDriverName string) strategy_service.IStrategyHandler[strategy_entry.StrategyVisitConfig, openapi_model.VisitInfoOutputConf] {
	h := &visitHandler{
