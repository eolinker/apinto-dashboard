...
type IWarnHistoryService interface {
	QueryList(ctx context.Context, namespaceId, partitionId, pageNum, pageSize int, startTime, endTime time.Time, name string) ([]*monitor_model.WarnHistoryInfo, int64, error)
	Create(ctx context.Context, namespaceId int, partitionId int, infos ...*monitor_model.WarnHistoryInfo) error
}

type warnHistoryService struct {
	warnHistoryStore monitor_store.IWarnHistoryIStore
}
...
func (w *warnHistoryService) Create(ctx context.Context, namespaceId int, partitionId int, infos ...*monitor_model.WarnHistoryInfo) error {
	if len(infos) == 0 {
		return nil
	}
...
func (w *warnHistoryService) QueryList(ctx context.Context, _, partitionId, pageNum, pageSize int, startTime, endTime time.Time, name string) ([]*monitor_model.WarnHistoryInfo, int64, error) {
	list, count, err := w.warnHistoryStore.GetPage(ctx, partitionId, name, pageNum, pageSize, startTime, endTime)
	if err != nil {
		return nil, 0, err
	}

	warnHistory := make([]*monitor_model.WarnHistoryInfo, 0, len(list))
	for _, history := range list {
		warnHistory = append(warnHistory, &monitor_model.WarnHistoryInfo{
