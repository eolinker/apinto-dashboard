...
import (
	...
	"github.com/eolinker/apinto-dashboard/model/bussiness-model"
	"github.com/eolinker/apinto-dashboard/store/system-store"
...
type IBussinessAuthService interface {
	GetActivationInfo(ctx context.Context) (*bussiness_model.ActivationInfo, error)
	ActivateCert(ctx context.Context, certFile []byte) (*bussiness_model.ActivationInfo, error)
	ReActivateCert(ctx context.Context, certFile []byte) (*bussiness_model.ActivationInfo, error)
	GetMachineCode(ctx context.Context) (string, error)
	CheckCertValid(ctx context.Context) (bool, error)
	examineCert(ctx context.Context, certInfo *bussiness_model.BussinessCertInfo, mac string) error
}

type bussinessAuthService struct {
	systemInfoStore system_store.ISystemInfoStore
...
func (b *bussinessAuthService) GetActivationInfo(ctx context.Context) (*bussiness_model.ActivationInfo, error) {
	certData, err := b.systemInfoStore.GetSystemInfoByKey(ctx, enum.CertDBKey)
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			return &bussiness_model.ActivationInfo{
				BussinessCertInfo: bussiness_model.BussinessCertInfo{},
				DashboardID:       "",
			}, nil
		}
		return nil, err
	}
	cert := new(bussiness_model.BussinessCertInfo)
...
func (b *bussinessAuthService) ActivateCert(ctx context.Context, certFile []byte) (*bussiness_model.ActivationInfo, error) {
	//校验证书
	machineCode := machine_code.GetMachineCode()
	certInfo := new(bussiness_model.BussinessCertInfo)
...
func (b *bussinessAuthService) ReActivateCert(ctx context.Context, certFile []byte) (*bussiness_model.ActivationInfo, error) {
	//校验证书
	machineCode := machine_code.GetMachineCode()
	certInfo := new(bussiness_model.BussinessCertInfo)
	err := yaml.Unmarshal(certFile, certInfo)
...
func (b *bussinessAuthService) getCertInfo(ctx context.Context) (*bussiness_model.BussinessCertInfo, error) {
...
// ExamineCert 验证证书
func (b *bussinessAuthService) examineCert(ctx context.Context, certInfo *bussiness_model.BussinessCertInfo, mac string) error {
	//校验机器码
...
func getSignParam(certInfo *bussiness_model.BussinessCertInfo) string {
	SignParam := &bussiness_model.CertSignatureInfo{
