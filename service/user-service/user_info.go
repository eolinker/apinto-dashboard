...
import (
	...
	"github.com/eolinker/apinto-dashboard/store/role-store"
	"github.com/eolinker/apinto-dashboard/store/user-store"
...
type IUserInfoService interface {
	...
	GetUserInfoList(ctx context.Context, roleId, keyWord string) ([]*user_model.UserInfo, error)
	GetUserInfoAll(ctx context.Context) ([]*user_model.UserInfo, error)
	...
	GetRoleList(ctx context.Context, userID int) ([]*user_model.RoleListItem, int, error)
	GetRoleInfo(ctx context.Context, roleID string) (*user_model.RoleInfo, error)
	GetRoleOptions(ctx context.Context) ([]*user_model.RoleOptionItem, error)
...
type userInfoService struct {
	...
	userRoleStore      user_store.IUserRoleStore
	roleStore          role_store.IRoleStore
	roleAccessStore    role_store.IRoleAccessStore
	roleAccessLogStore role_store.IRoleAccessLogStore
...
func (u *userInfoService) GetUserInfoAll(ctx context.Context) ([]*user_model.UserInfo, error) {
...
func (u *userInfoService) GetUserInfoList(ctx context.Context, roleUUID, keyword string) ([]*user_model.UserInfo, error) {
...
func (u *userInfoService) DelUser(ctx context.Context, operator int, userIds []int) error {
	...
	common.SetGinContextAuditObject(ctx, &audit_model.LogObjectInfo{Uuid: strings.Join(uuids, ","), Name: strings.Join(names, ",")})
...
func (u *userInfoService) GetUserInfo(ctx context.Context, userID int) (*user_model.UserInfo, error) {
	...
	if userInfo != nil {
		return &user_model.UserInfo{UserInfo: userInfo}, err
	}
	...
	return &user_model.UserInfo{UserInfo: userInfo}, nil
}

func (u *userInfoService) GetUserInfoByUUID(ctx context.Context, userID string) (*user_model.UserInfo, error) {
...
func (u *userInfoService) GetRoleList(ctx context.Context, userID int) ([]*user_model.RoleListItem, int, error) {
	//获取内置角色
	...
	roleList := make([]*user_model.RoleListItem, 0, len(buildInRoles)+len(roles))
...
func (u *userInfoService) GetRoleInfo(ctx context.Context, roleUUID string) (*user_model.RoleInfo, error) {
	if access.IsBuildInRole(roleUUID) {
		role := access.GetBuildInRole(roleUUID)
		return &user_model.RoleInfo{
			Title:  role.Title,
			Desc:   role.Desc,
			Access: role.Access,
		}, nil
...
func (u *userInfoService) GetRoleOptions(ctx context.Context) ([]*user_model.RoleOptionItem, error) {
	//获取内置角色
	buildInRoles := access.GetBuildInRoleList()
	//获取数据库表中的自定义角色
	roles, err := u.roleStore.GetAllRole(ctx)
	...
	roleList := make([]*user_model.RoleOptionItem, 0, len(buildInRoles)+len(roles))

	for _, role := range buildInRoles {
		item := &user_model.RoleOptionItem{
			ID:             role.Uuid,
			Title:          role.Title,
			OperateDisable: !role.IsAddable,
		}
...
func (u *userInfoService) UpdateRole(ctx context.Context, userID int, roleUUID string, input *user_dto.ProxyRoleInfo) error {
	...
	return u.roleStore.Transaction(ctx, func(txCtx context.Context) error {
		...
		//更新角色缓存
		roleAccessCacheConf := access_model.CreateRoleAccess(accessList...)
...
func (u *userInfoService) RoleBatchUpdate(ctx context.Context, userIds []int, roleUUID string) error {
	...
	common.SetGinContextAuditObject(ctx, &audit_model.LogObjectInfo{Uuid: strings.Join(userIdStrs, ","), Name: strings.Join(userNames, ",")})
	return nil
}
