...
import (
	...
	"github.com/eolinker/apinto-dashboard/store/cluster-store"
...
type clusterService struct {
	clusterStore           cluster_store.IClusterStore
	clusterHistoryStore    cluster_store.IClusterHistoryStore
	clusterRuntime         cluster_store.IClusterRuntimeStore
	clusterNodeService     IClusterNodeService
	clusterVariableService variable_service.IClusterVariableService
...
func (c *clusterService) GetAllCluster(ctx context.Context) ([]*cluster_model.Cluster, error) {
	clusters, err := c.clusterStore.GetAll(ctx)
	if err != nil {
		return nil, err
	}
	list := make([]*cluster_model.Cluster, 0, len(clusters))
	for _, cluster := range clusters {
		item := &cluster_model.Cluster{
			Cluster: cluster,
		}
...
func (c *clusterService) GetByNamespaceByName(ctx context.Context, namespaceId int, name string) (*cluster_model.Cluster, error) {
	cluster, err := c.clusterStore.GetByNamespaceByName(ctx, namespaceId, name)
	if err != nil {
	...
	return &cluster_model.Cluster{Cluster: cluster}, nil
}

// GetByClusterId 根据集群ID获取集群信息
func (c *clusterService) GetByClusterId(ctx context.Context, clusterId int) (*cluster_model.Cluster, error) {
	cluster, err := c.clusterStore.Get(ctx, clusterId)
	if err != nil {
		return nil, errors.New("cluster does not exist")
	}

	value := &cluster_model.Cluster{
		Cluster: cluster,
	}
	return value, nil
}

func (c *clusterService) GetByNamespaceId(ctx context.Context, namespaceId int) ([]*cluster_model.Cluster, error) {
...
func (c *clusterService) GetByNames(ctx context.Context, namespaceId int, names []string) ([]*cluster_model.Cluster, error) {
...
func (c *clusterService) Insert(ctx context.Context, namespaceId, userId int, clusterInput *cluster_dto.ClusterInput) error {
	...
	nodes := make([]*cluster_model.ClusterNode, 0)
	...
	return c.clusterStore.Transaction(ctx, func(txCtx context.Context) error {
		...
		entryClusterNodes := make([]*cluster_model.ClusterNode, 0, len(nodes))
		nodesAdminAddr := make([]string, 0, len(nodes))

		for _, node := range nodes {
			entryClusterNodes = append(entryClusterNodes, &cluster_model.ClusterNode{ClusterNode: &cluster_entry.ClusterNode{
				NamespaceId: namespaceId,
...
func (c *clusterService) QueryByNamespaceId(ctx context.Context, namespaceId int, clusterName string) (*cluster_model.Cluster, error) {
...
func (c *clusterService) QueryListByNamespaceId(ctx context.Context, namespaceId int) ([]*cluster_model.Cluster, error) {
	clusters, err := c.clusterStore.GetAllByNamespaceId(ctx, namespaceId)
	if err != nil {
		return nil, err
	}
