...
func (c *clusterNodeService) Insert(ctx context.Context, nodes []*cluster_model.ClusterNode) error {
...
func (c *clusterNodeService) QueryList(ctx context.Context, namespaceId int, clusterName string) ([]*cluster_model.ClusterNode, bool, error) {
	cluster, err := c.clusterService.GetByNamespaceByName(ctx, namespaceId, clusterName)
	if err != nil {
		return nil, false, err
	}

	list := make([]*cluster_model.ClusterNode, 0)
	...
	group.Go(func() error {
		...
		for _, node := range nodes {
			list = append(list, &cluster_model.ClusterNode{
				ClusterNode: node,
	...
	clusterNodes := make([]*cluster_model.ClusterNode, 0)
	group.Go(func() error {
...
func (c *clusterNodeService) QueryByClusterIds(ctx context.Context, clusterIds ...int) ([]*cluster_model.ClusterNode, error) {
	nodes, err := c.clusterNodeStore.GetAllByClusterIds(ctx, clusterIds...)
	if err != nil {
		return nil, err
	}
	list := make([]*cluster_model.ClusterNode, 0, len(nodes))
...
func (c *clusterNodeService) nodeRepeatContrast(ctx context.Context, namespaceId, clusterId int, newList []*cluster_model.ClusterNode) error {
	clusters, err := c.clusterService.GetByNamespaceId(ctx, namespaceId)
	if err != nil {
		return err
	}

	clustersMap := common.SliceToMap(clusters, func(t *cluster_model.Cluster) int {
		return t.Id
	})
	...
	mapNode := common.SliceToMap(newList, func(t *cluster_model.ClusterNode) string {
		return t.Name
	})
...
func (c *clusterNodeService) GetNodesByUrl(addr string) ([]*cluster_model.ClusterNode, error) {
